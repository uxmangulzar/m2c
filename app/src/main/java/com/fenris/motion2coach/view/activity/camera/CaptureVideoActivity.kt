...
import com.fenris.motion2coach.util.Helper.convertDateToLong
import com.fenris.motion2coach.util.Helper.convertTimeToLong
...
import com.fenris.motion2coach.util.camera2utils.getPreviewOutputSize
...
@AndroidEntryPoint
class CaptureVideoActivity : AppCompatActivity(),Animation.AnimationListener {
    private var fpsRangesFront: Int=30
...
//    private var animMoveToTop: Animation? = null
    ...
    var fpsRanges: Int? = 30
    var resolutionSelected1: String? = "1280"
    var resolutionSelected2: String? = "720"
    ...
    /** AndroidX nmp4gation arguments */
    private var backgroundHandler: Handler? = null
    var screenPause = false
    private val yuvBytes = arrayOfNulls<ByteArray>(3)
    private var rgbBytes: IntArray? = null
    /** Host's nmp4gation controller */
    ...
    private fun mute() {
        param.putFloat(KEY_PARAM_VOLUME, 0.0f)
    }

    fun unmute() {
        param.putFloat(KEY_PARAM_VOLUME, 1.0f)
    }
...
//            fragmentBinding.overlay.foreground = Color.argb(150, 255, 255, 255).toDrawable()
            ...
            viewBinding!!.overlay.postDelayed({
                if (currentlyRecording) {
                    // Remove white flash animation
                    viewBinding!!.overlay.foreground = null
                    // Restart animation recursively
                    if (currentlyRecording) {
                        viewBinding!!.overlay.postDelayed(animationTask,
    ...
    private val previewRequest: CaptureRequest by lazy {
        // Capture request holds
        // references to target surfaces
        session.device.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW).apply {
            previewBuilder = this
            // Add the preview surface target
            addTarget(viewBinding!!.viewFinder.holder.surface)
            //            ISO
            if (SessionManager.getStringPref(HelperKeys.ISO_FRONT,applicationContext)!=""&&mCameraId=="1"){
                set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO )
//                set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF)
                set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)

                set(CaptureRequest.SENSOR_SENSITIVITY, SessionManager.getStringPref(HelperKeys.ISO_FRONT,applicationContext).toInt())
            }
            if (SessionManager.getStringPref(HelperKeys.ISO_BACK,applicationContext)!=""&&mCameraId=="0"){
                set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO )
//                set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF)
                set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)

                set(CaptureRequest.SENSOR_SENSITIVITY, SessionManager.getStringPref(HelperKeys.ISO_BACK,applicationContext).toInt())
            }
//            Shutter speed
            if (SessionManager.getStringPref(HelperKeys.SS_BACK,applicationContext)!=""&&mCameraId=="0"){
                set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO )
//                set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF)
                set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)

                set(CaptureRequest.SENSOR_EXPOSURE_TIME, SessionManager.getStringPref(HelperKeys.SS_BACK,applicationContext).toLong())
            }
            if (SessionManager.getStringPref(HelperKeys.SS_FRONT,applicationContext)!=""&&mCameraId=="1"){
                set(CaptureRequest.CONTROL_AWB_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO )
//                set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF)
                set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_OFF)

                set(CaptureRequest.SENSOR_EXPOSURE_TIME, SessionManager.getStringPref(HelperKeys.SS_FRONT,applicationContext).toLong())
            }
...
//                set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_OFF)
            ...
            if (mCameraId=="0"){
                set(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, Range(fpsRanges!!, fpsRanges!!))
            }else{
...
//        0
    ...
    override fun onRequestPermissionsResult(
        ...
        if (allPermissionsGranted()) {
            ...
            Helper.openPermissionSettings(this@CaptureVideoActivity)
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressDialog = android.app.ProgressDialog(this@CaptureVideoActivity)
...
//        if (SessionManager.getStringPref(HelperKeys.WEIGHT,applicationContext)==""){
...
//        if(mCameraId=="1"){
//            fpsRanges=fpsRangesFront
//        }

        val fpsResolution=SessionManager.getStringPref(HelperKeys.FPS_RESOLUTION,applicationContext)
        if (!fpsResolution.equals("")){
            val splitArray: List<String> = fpsResolution.split("x")
            resolutionSelected1 = splitArray[0]
            resolutionSelected2 = splitArray[1]
        }
...
//        if (recordType == "down"){
//            animMoveToTop = AnimationUtils.loadAnimation(applicationContext, R.anim.vertical)
//        }else if (recordType == "face"){
//            animMoveToTop = AnimationUtils.loadAnimation(applicationContext, R.anim.horizontal)
//        }
...
//        animMoveToTop!!.repeatCount = Animation.INFINITE
//        animMoveToTop!!.duration = 6000

        if (recordType == "down"){
//            viewBinding!!.ivAnimateRectVertical.visibility =View.VISIBLE
//            viewBinding!!.ivLineVertical.visibility =View.VISIBLE
//            viewBinding!!.ivAnimateRectVertical.startAnimation(animMoveToTop)
        }else if (recordType == "face"){
//            viewBinding!!.ivAnimateRectHorizontal.visibility =View.VISIBLE
//            viewBinding!!.ivLineHorizontal.visibility =View.VISIBLE
//            viewBinding!!.ivAnimateRectHorizontal.startAnimation(animMoveToTop)
        }
        var x = 0.0
        var y = 0.0
        ...
        if (!SessionManager.getStringPref(HelperKeys.TIMER_TIME,applicationContext).equals("")){
            timerTime=SessionManager.getStringPref(HelperKeys.TIMER_TIME,applicationContext).toInt()
            viewBinding!!.tvTimershow.text = timerTime.toString()+"s"
            timerTime += 1

        }else{
            timerTime=4
            viewBinding!!.tvTimershow.text = "3s"
        ...
        viewBinding!!.circularCountDownView.setOnTickListener(object : HappyTimer.OnTickListener {
            ...
            override fun onTick(completedSeconds: Int, remainingSeconds: Int) {
                ...
                if (remainingSeconds!=0){
                    textToSpeechSystem!!.speak(""+remainingSeconds, TextToSpeech.QUEUE_ADD, param,null)
                }
            }
...
//            startActivity(Intent(applicationContext, HistoryActivity::class.java))
        ...
        viewBinding!!.ivClub.setOnClickListener {
            if (SessionManager.getStringPref(HelperKeys.SWING_TYPE_ID,applicationContext)=="1") {
                openClubTypeDialog(this@CaptureVideoActivity)
            }
        }
...
//            }
            if (SessionManager.getStringPref(HelperKeys.CONT_RECORD, applicationContext)
                ...
                val videoModel= Video(0, file?.path, convertDateToLong(),
                    convertTimeToLong(),
                    "saved"
                    ,mimeType
        ...
        viewBinding!!.tvTenSec.setOnClickListener {
            ...
            timerTime=10
            SessionManager.putStringPref(HelperKeys.TIMER_TIME,timerTime.toString(),applicationContext)
...
//            val intent=Intent(applicationContext, VisualizeActivity::class.java)
...
//                    con.startActivity(intent)
                ...
                val url ="https://m2c-media.s3.eu-central-1.amazonaws.com/"+newString+"_vis.mp4"
                Log.e("TAG",url)
...
//                val newString: String = public_url!!.replace("mp4", "json")
//
//                var intent=Intent(applicationContext,ReportActivity::class.java)
//                intent.putExtra("url",newString)
//                startActivity(intent)
            ...
            if(stepPassed==4){
                val newString: String = fileName.replace(".mp4", "")
                val url ="https://m2c-media.s3.eu-central-1.amazonaws.com/"+newString+"_overlay.mp4"
                Log.e("TAG",url)
...
//            .setMessage("Your camera is supporting 30fps . Do you want to continue video recording?")
    ...
    private fun audioSetting() {
        if (SessionManager.getStringPref(HelperKeys.MUTE_STATE, applicationContext)
            ...
            viewBinding!!.ivAudio.setImageDrawable(resources.getDrawable(R.drawable.ic_vector_mute,null))
            mute()
            ...
            viewBinding!!.ivAudio.setImageDrawable(resources.getDrawable(R.drawable.ic_vector_unmute,null))
            unmute()
        }
    }
    fun audioSettingDefault() {
        if (SessionManager.getStringPref(HelperKeys.MUTE_STATE, applicationContext)
                .equals("")
        ) {
    ...
    private fun captureCoreVIdeo() {
        if(flag_start_end){
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                // repeating requests without hmp4ng to explicitly call `session.stopRepeating`
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                mimeType =
                    MimeTypeMap.getSingleton().getMimeTypeFromExtension(outputFile.extension).toString()
                val authority = "${BuildConfig.APPLICATION_ID}.provider"
                val data = FileProvider.getUriForFile(applicationContext, authority, outputFile)
                ...
                file = saveVideoToAppScopeStorage(applicationContext, data, mimeType)
...
//                        hello
                        ...
                        viewBinding!!.stop.setImageDrawable(resources.getDrawable(R.drawable.video_play_button,null))
                        viewBinding!!.stop.visibility = View.INVISIBLE
...
//                            viewBinding!!.ivLineHorizontal.visibility=View.GONE
                        ...
                        uploadMedia(mimeType,File(file!!.path).name,file!!.path,0,
                            Video(0, file?.path, convertDateToLong(),
                            convertTimeToLong(),
                            "saved"
                            ,mimeType
                            ,SessionManager.getStringPref(HelperKeys.ORIENTATION,applicationContext)
...
//                if (!screenPause){
                    ...
                    Log.d(TAG, "View finder size: ${viewBinding!!.viewFinder.width} x ${viewBinding!!.viewFinder.height}")
...
//                    viewBinding!!.firstbottom.visibility=View.GONE
//                    viewBinding!!.firsttop.visibility=View.GONE
                    viewBinding!!.secondbottom.visibility=View.VISIBLE
//                    viewBinding!!.secondtop.visibility=View.VISIBLE
    ...
    private fun createEncoder(): EncoderWrapper {
        val videoEncoder = when {
            1L == 1L-> MediaFormat.MIMETYPE_VIDEO_AVC
            1L < 4096L -> MediaFormat.MIMETYPE_VIDEO_HEVC
        ...
        return EncoderWrapper(resolutionSelected1!!.toInt(), resolutionSelected2!!.toInt(), RECORDER_VIDEO_BITRATE, fpsRanges!!,
                orientation, videoEncoder, codecProfile, outputFile)

//        return EncoderWrapper(720, 576, RECORDER_VIDEO_BITRATE, fpsRanges!!,
//                orientation, videoEncoder, codecProfile, outputFile)
    }
    ...
    private fun initializeCamera() = lifecycleScope.launch(Dispatchers.Main) {
        ...
        camera = openCamera(cameraManager, cameraId = mCameraId, cameraHandler)

        // Creates list of Surfaces where the camera will output frames
        val targets = listOf(viewBinding!!.viewFinder.holder.surface, encoderSurface)
    ...
    private fun saveVideoToAppScopeStorage(context: Context, videoUri: Uri?, mimeType: String?): File? {
        if(videoUri == null || mimeType == null){
            return null
        }
        val timeDate=System.currentTimeMillis()
        val usrId = SessionManager.getStringPref(HelperKeys.USER_ID, applicationContext)
        val und ="-"
        val fileName = "m2c-and-$usrId$und$timeDate.${mimeType.substring(mimeType.indexOf("/") + 1)}"
        val moviesDir = filesDir
...
//        }
    ...
    private fun enumerateHighSpeedCamerasBack(cameraManager: CameraManager): List<CameraInfo> {
        ...
        if (!checked){
            openFpsDialog(this@CaptureVideoActivity)
        }
    ...
    private fun openFpsDialog(activity: Activity){
    ...
    fun onTouch( event: MotionEvent): Boolean {
        try {
            val activity: Activity = this@CaptureVideoActivity
...
//                        showDialog(this@CaptureVideoActivity)
    ...
    private fun addVideoToServer(url: String, fileName: String, index: Int, videoModel: Video) {
        ...
        val sessionsModel = SessionsUploadRequestModel(
            ...
            "",
            "",
            SessionManager.getStringPref(HelperKeys.HEIGHT,applicationContext).toString().toFloat(),
            SessionManager.getStringPref(HelperKeys.WEIGHT,applicationContext).toString().toFloat(),
